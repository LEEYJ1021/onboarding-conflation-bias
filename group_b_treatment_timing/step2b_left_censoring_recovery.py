"""
================================================================================
Step 2b — Left-censoring recovery, pre-window safety check, final cohort
================================================================================
Given step2a's result (7 pure new adopters, 30 left-censored, verdict
NOT_VIABLE), this script:

    A. Coverage check    — does the raw (unfiltered) panel extend further
                            back than the stable window's start date for
                            each left-censored customer? Distinguishes
                            "structural" censoring from censoring that is
                            an artifact of the stable-window filter.
    B. True first-date recomputation — re-derive the actual first-adoption
                            date from the raw campaign-type panel, including
                            pre-window days.
    C. Safety verification — before splicing pre-window data into the
                            event-time panel, verify it on four criteria
                            (anomaly flags / continuity / volatility /
                            extreme-value density) and label each recovered
                            case SAFE / CAUTION / UNSAFE.
    D. Final cohort assembly — combine within-window adopters + recovered
                            SAFE + recovered CAUTION into
                            primary_adopter / robustness_only_adopter /
                            still_left_censored / never_treated.
    E. Outcome splicing + PRIMARY (SAFE only) and ROBUST (SAFE+CAUTION)
                            event-time panels and event-study plots.

Inputs:  customer_day_panel.csv, customer_day_campaign_type_panel.csv,
         df_analysis_master.csv,
         staggered_adoption_customer_level_type{T}.csv (step2a output)
Outputs: left_censoring_diagnosis_type{T}.json
         pre_window_safety_check_type{T}.csv / _summary.json
         staggered_adoption_FINAL_type{T}.csv
         event_time_panel_FINAL_type{T}.csv / event_time_panel_ROBUST_type{T}.csv
         event_study_FINAL_type{T}.png / event_study_ROBUST_type{T}.png
...
ORIG_ADOPTION_PATH = STEP2_OUT / f"staggered_adoption_customer_level_type{TARGET_TYPE}.csv"
for p in (PANEL_PATH, CTP_PATH, MASTER_PATH, ORIG_ADOPTION_PATH):
    if not p.exists():
        raise FileNotFoundError(f"Missing input: {p} — run step2a first.")
...
print(f"Left-censored customers: {len(LEFT_CENSORED)} (type {TARGET_TYPE})\n")

# ============================================================================
# A. Raw panel coverage vs. stable-window start
# ============================================================================
print("=" * 70)
print("A. Raw panel date coverage vs. stable-window start")
print("=" * 70)
...
coverage_check["has_pre_window_data"] = coverage_check["raw_date_min"] < coverage_check["customer_window_start"]
coverage_check["days_available_before_window"] = (
    coverage_check["customer_window_start"] - coverage_check["raw_date_min"]).dt.days.clip(lower=0)
n_recoverable_candidates = int(coverage_check["has_pre_window_data"].sum())
print(f"Customers with data preceding the stable window: {n_recoverable_candidates} / {len(coverage_check)}")
if n_recoverable_candidates == 0:
    print("\nNo pre-window data at all in the raw panel — structural left-censoring confirmed.")

# ============================================================================
# B. True first-adoption date, recomputed from the raw campaign-type panel
# ============================================================================
print("\n" + "=" * 70)
print("B. Re-deriving the true first-adoption date (including pre-window days)")
print("=" * 70)
recovered_rows = []
for _, row in LEFT_CENSORED.iterrows():
    cid, tcode = row["customer_id"], row["campaign_type"]
    sub = ctp[(ctp["customer_id"] == cid) & (ctp["campaign_type"] == tcode) & (ctp["cost"] > 0)]
    if sub.empty:
        recovered_rows.append({"customer_id": cid, "campaign_type": tcode,
    ...
    status = "recovered_earlier_than_stable_window" if true_first < w_start else "consistent_with_stable_window"
    recovered_rows.append({"customer_id": cid, "campaign_type": tcode,
...
print(f"True first-adoption date earlier than stable-window start: {n_actually_recovered} / {len(recovered_df)}")
...
if n_recoverable_candidates == 0 and n_actually_recovered == 0:
    ...
    diag_msg = ("No pre-window data exists, or it exists but is already active — structural "
                "left-censoring is confirmed; recovery is not possible.")
elif n_actually_recovered > 0:
    diag_verdict = "PARTIALLY_RECOVERABLE"
    diag_msg = (f"{n_actually_recovered} case(s) have a true adoption date earlier than the "
                f"stable-window start recoverable from the raw campaign-type panel. Because this "
                f"segment lies outside the stable window, its data quality must be separately "
                f"verified before splicing it into the event-time panel (step C below).")
else:
    diag_verdict = "NO_ADDITIONAL_RECOVERY"
    diag_msg = "Recovery candidates existed but no additional recovery was confirmed."
print(f"\nVerdict: {diag_verdict}\n{diag_msg}")

diag_path = STEP2_OUT / f"left_censoring_diagnosis_type{TARGET_TYPE}.json"
with open(diag_path, "w", encoding="utf-8") as f:
    json.dump({"n_left_censored": int(len(LEFT_CENSORED)), "n_pre_window_data_available": n_recoverable_candidates,
               "n_actually_recovered_earlier_date": n_actually_recovered, "verdict": diag_verdict,
               "verdict_message": diag_msg}, f, ensure_ascii=False, indent=2, default=str)
print(f"\nDiagnosis saved: {diag_path}")

# ============================================================================
# C. Pre-window safety verification (SAFE / CAUTION / UNSAFE)
# ============================================================================
print("\n" + "=" * 70)
print("C. Pre-window splice safety verification")
print("=" * 70)
...
TARGET_SAFETY = TARGET_SAFETY.merge(
    LEFT_CENSORED[["customer_id", "campaign_type", "customer_window_start"]], on=["customer_id", "campaign_type"], how="left")
print(f"Cases to verify: {len(TARGET_SAFETY)}\n")
...
for _, row in TARGET_SAFETY.iterrows():
    ...
    stable_ref = master[(master["customer_id"] == cid) & (master["date"] >= w_start)].sort_values("date").head(30)
    if pre.empty:
    ...
    safety_results.append({
        "customer_id": cid, "campaign_type": row["campaign_type"], "true_first_active_date": true_first,
        "customer_window_start": w_start, "pre_window_days_span": expected_days,
        "pre_window_days_observed": actual_days,
...
safety_path = STEP2_OUT / f"pre_window_safety_check_type{TARGET_TYPE}.csv"
...
with open(STEP2_OUT / f"pre_window_safety_summary_type{TARGET_TYPE}.json", "w", encoding="utf-8") as f:
    json.dump({"n_checked": int(len(safety_df)), "n_safe": n_safe, "n_caution": n_caution, "n_unsafe": n_unsafe},
               f, ensure_ascii=False, indent=2, default=str)
print(f"\nSaved: {safety_path}\nSAFE={n_safe} / CAUTION={n_caution} / UNSAFE={n_unsafe}")

# ============================================================================
# D. Final cohort assembly
# ============================================================================
print("\n" + "=" * 70)
...
recovered_map = recovered_df.set_index(["customer_id", "campaign_type"])[
    ["true_first_active_date", "recovery_status"]].to_dict("index")

final_rows = []
for _, row in orig.iterrows():
    cid, tcode = row["customer_id"], row["campaign_type"]
    if row["cohort_type"] == "adopted_within_window":
        final_rows.append({"customer_id": cid, "campaign_type": tcode,
        ...
        final_rows.append({"customer_id": cid, "campaign_type": tcode, "final_first_treated_date": pd.NaT,
        ...
        if rec_status == "recovered_earlier_than_stable_window" and verdict == "SAFE":
            final_rows.append({"customer_id": cid, "campaign_type": tcode, "final_first_treated_date": rec_date,
...
final_path = STEP2_OUT / f"staggered_adoption_FINAL_type{TARGET_TYPE}.csv"
final_df.to_csv(final_path, index=False)
print(f"\n[type {TARGET_TYPE}] final cohort distribution:")
final_counts = final_df["final_cohort"].value_counts()
print(final_counts)
print(f"\nSaved final cohort table: {final_path}")
...
# ============================================================================
# E. Extended outcome computation + PRIMARY/ROBUST event-time panels
# ============================================================================
print("\n" + "=" * 70)
...
def build_event_panel_and_plot(cohort_set, tag):
    coh = final_df[final_df["final_cohort"].isin(cohort_set)][["customer_id", "final_first_treated_date"]].dropna()
    if coh.empty:
        print(f"[{tag}] no eligible customers — skipping")
        return None
    ...
    ev_path = STEP2_OUT / f"event_time_panel_{tag}_type{TARGET_TYPE}.csv"
    panel_t.to_csv(ev_path, index=False)
    n_adopters = coh["customer_id"].nunique()
    print(f"[{tag}] Saved event-time panel: {ev_path} ({n_adopters} adopters, +/-{EVENT_WINDOW} days, {len(panel_t):,} rows)")
    ...
    for j, ycol in enumerate(OUTCOME_VARS):
        ...
        ax.legend()
    plt.tight_layout()
    fig_path = STEP2_OUT / f"event_study_{tag}_type{TARGET_TYPE}.png"
...
summary = {
    ...
    "final_cohort_counts": {"primary_adopter": n_primary, "robustness_only_adopter": n_robust_only,
                             "still_left_censored": n_still_censored, "never_treated": n_never},
    "event_window_days": EVENT_WINDOW,
    "event_time_panel": {"primary_FINAL": r_primary, "robust_ROBUST": r_robust},
}
summary_path = STEP2_OUT / f"staggered_adoption_FINAL_type{TARGET_TYPE}_summary.json"
with open(summary_path, "w", encoding="utf-8") as f:
    json.dump(summary, f, ensure_ascii=False, indent=2, default=str)
print("\n" + "=" * 70)
print(f"Done. 1st-attempt pure new-adopters -> final primary_adopter {n_primary} (robust version {n_primary + n_robust_only})")
print(f"Summary saved: {summary_path}")
print("Next step: step2c_pretrend_test.py (formal pre-trend test on the FINAL event-time panel)")
